  ...
  const fetchStoryDetails = useCallback(async (showRefreshing = false) => {
    try {
      ...
      if (response?.status === true && response?.data) {
        ...
        // If no images at all, leave empty
            ...
            {isYouTubeUrl(storyDetails.videoUri) ? (
              ...
              <TouchableOpacity
                ...
                {typeof storyDetails.thumbnailUri === 'object' && storyDetails.thumbnailUri?.uri && typeof storyDetails.thumbnailUri.uri === 'string' ? (
                  ...
                  <FastImage
                    source={{
                      uri: getYouTubeThumbnailUrl(storyDetails.videoUri) || '',
                      priority: FastImage.priority.normal,
                      cache: FastImage.cacheControl.immutable,
                    }}
                    style={{width: '100%', height: '100%'}}
          ...
          storyDetails.images && storyDetails.images.length > 0 && (
            <View style={dynamicStyles.videoContainer}>
              <TouchableOpacity
                ...
                activeOpacity={0.7}>
                {typeof storyDetails.images[0] === 'object' && storyDetails.images[0]?.uri && typeof storyDetails.images[0].uri === 'string' ? (
                  <FastImage
                    source={{
          ...
          {((storyDetails.videoUri && storyDetails.videoUri.trim() !== '' && previewImages.length > 0) || 
            ...
            <View style={dynamicStyles.thumbnailRow}>
              {(storyDetails.videoUri && storyDetails.videoUri.trim() !== '' 
                ...
                const actualIndex = (storyDetails.videoUri && storyDetails.videoUri.trim() !== '') ? index : index + 1;
                // Calculate remaining count based on whether we're skipping first image
                const startIndex = (storyDetails.videoUri && storyDetails.videoUri.trim() !== '') ? 0 : 1;
